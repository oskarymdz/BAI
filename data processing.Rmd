...
# Load original data
df_original <- read_csv("data/df_ULTRA_enhanced_with_filters.csv")

# Address extraction function
extract_street_complete <- function(address) {
  if (is.na(address) || str_trim(address) == "") return(NA_character_)
  
  address_clean <- str_trim(address)
  
  # list of Melbourne street types
  street_types <- c(
    "Boulevard", "Parade", "Avenue", "Street", "Road", 
    "Lane", "Place", "Court", "Drive", "Walk", 
    "Terrace", "Highway", "Way", "Crescent", "Square",
    "Mall", "Arcade", "Circuit", "Grove", "Close",
    "Esplanade", "Promenade", "Circus", "Quay", 
    "Alley", "Market", "Bridge", "Freeway"
  )
  
  # Pattern: Street name + type (ignoring unit/level prefixes)
  pattern <- paste0(
    "([A-Za-z][A-Za-z\\s'-]+?)\\s+(",
    paste(street_types, collapse = "|"),
    ")(?:\\s+(East|West|North|South))?\\b"
  )
  
  # Try direct extraction
  match <- str_extract(address_clean, regex(pattern, ignore_case = TRUE))
  if (!is.na(match)) {
    return(str_to_title(str_trim(match)))
  }
  
  # If has comma, try each part
  if (str_detect(address_clean, ",")) {
    parts <- str_split(address_clean, ",")[[1]]
    for (part in rev(parts)) {  # Try from end first
      match <- str_extract(str_trim(part), regex(pattern, ignore_case = TRUE))
      if (!is.na(match)) {
        return(str_to_title(str_trim(match)))
      }
    }
  ...
  # After unit/level prefix
  after_prefix <- str_remove(address_clean, 
    regex("^(Part|Shop|Unit|Level|Ground|Floor|Tenancy|Restaurant|Shed|Kiosk|Rear)\\s*[\\d,\\s-]*", 
          ignore_case = TRUE))
  
  match <- str_extract(after_prefix, regex(pattern, ignore_case = TRUE))
  if (!is.na(match)) {
    return(str_to_title(str_trim(match)))
  }
...
# Apply to ALL data
df_fixed <- df_original %>%
  mutate(
    street_address = map_chr(business_address, extract_street_complete),
    street_type = map_chr(street_address, function(s) {
      if (is.na(s)) return(NA_character_)
      types <- c("Boulevard", "Parade", "Avenue", "Street", "Road", "Lane", 
                 "Place", "Court", "Drive", "Walk", "Terrace", "Highway",
                 "Way", "Crescent", "Square", "Mall", "Arcade", "Circuit",
                 "Grove", "Close", "Esplanade", "Promenade", "Circus", 
                 "Quay", "Alley", "Market", "Bridge", "Freeway")
      for (type in types) {
        if (str_detect(s, regex(paste0("\\b", type, "\\b"), ignore_case = TRUE))) {
          return(type)
        }
      }
      return(NA_character_)
...
# Save
write_csv(df_fixed, "data/df_with_streets.csv")
