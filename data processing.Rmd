...
# Load data
df_original <- read_csv("data/df_enhanced_with_filters.csv")

# EXTRACTION 
extract_street_corrected <- function(address) {
  ...
  pattern <- paste0(
    ...
    ")\\b"  # REMOVED: (?:\\s+(East|West|North|South))?
  )
...
df <- read_csv("data/df_enhanced_with_filters.csv")

df_corrected <- df %>%
  mutate(
    street_address = map_chr(business_address, extract_street_corrected),
...
# Save 
write_csv(df_corrected, "data/df_with_streets.csv")
